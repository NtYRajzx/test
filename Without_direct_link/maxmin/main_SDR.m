...
%%
ts = tic();
rate_set = zeros(length(sigma2_dB_set),length(num_irs_elements_set));
rate_set_rand = zeros(length(sigma2_dB_set),length(num_irs_elements_set));
for iter_snr=1:length(sigma2_dB_set)
    ...
    for iter_m = 1:length(num_irs_elements_set)
        ...
        parfor ii=1:num_samples
            ...
            %bisection + sdr
            [v,obj_tmp] = sdr_maxmin(A,sigma2_dB);
        ...
        rate_set(iter_snr,iter_m) = rate_tmp/num_samples;
        rate_set_rand(iter_snr,iter_m) = rate_rnd_tmp/num_samples;
    end
end
