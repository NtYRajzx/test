...
for iter_snr=10:length(sigma2_dB_set)
    ...
    for ii=1:num_samples
        ...
        [v,obj_tmp] = subgradient_maxmin(A,sigma2_dB,v0);
        rate_tmp1 = rate_tmp1+compute_minrate(A, v, p, alpha, sigma2_dB);
                        

        % zf initialization
        v0 = alter_proj(A);
        rate_tmp4 = rate_tmp4+compute_minrate(A, v0, p, alpha, sigma2_dB);
        [v,obj_tmp] = subgradient_maxmin(A,sigma2_dB,v0);
        rate_tmp2 = rate_tmp2+compute_minrate(A, v, p, alpha, sigma2_dB);
        
        % zf initialization
        v0 = alter_proj_2(A);
        rate_tmp5 = rate_tmp5+compute_minrate(A, v0, p, alpha, sigma2_dB);
        [v, obj_tmp] = subgradient_maxmin(A,sigma2_dB,v0);
        rate_tmp3 = rate_tmp3+compute_minrate(A, v, p, alpha, sigma2_dB);
        
        % random irs
        v =exp(1j.*2*pi*rand(num_irs_elements,1));
        v = v./abs(v);
        rate_rnd_tmp = rate_rnd_tmp+compute_minrate(A, v, p, alpha, sigma2_dB);
    end
    ...
    save main_rate_SNR.mat
end
running_time = toc(ts)/60;
fprintf('running time: %3.3f mins\n',toc(ts)/60);
save main_rate_SNR.mat
%% plot
plot(power_set, rate_set1, 'o-'); hold on
plot(power_set, rate_set2, 's-'); hold on
plot(power_set, rate_set3, 'v-'); hold on
plot(power_set, rate_set4, '<-'); hold on
plot(power_set, rate_set5, '>-'); hold on
