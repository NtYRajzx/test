...
for iter_snr=1:length(sigma2_dB_set)
    ...
    parfor ii=1:num_samples
        ...
        v = mamnopt_func(A,B,p,alpha,sigma2_dB,v0);
        rate_tmp1 = rate_tmp1+compute_rate(A,B,v, p, alpha, sigma2_dB);
        
        % zf initialization
        v0 = alter_proj(A,B);
        rate_tmp4 = rate_tmp4+compute_rate(A,B,v0, p, alpha, sigma2_dB);
        v = mamnopt_func(A,B,p,alpha,sigma2_dB,v0);
        rate_tmp2 = rate_tmp2+compute_rate(A,B,v, p, alpha, sigma2_dB);
        
        % zf initialization
        v0 = alter_proj_2(A,B);
        rate_tmp5 = rate_tmp5+compute_rate(A, B, v0, p, alpha, sigma2_dB);
        v = mamnopt_func(A,B,p,alpha,sigma2_dB,v0);
        rate_tmp3 = rate_tmp3+compute_rate(A, B, v, p, alpha, sigma2_dB);
...
save main_rate_SNR_manopt2_1.mat
%% plot
