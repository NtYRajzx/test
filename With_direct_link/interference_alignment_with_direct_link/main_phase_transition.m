...
% rng(1)
num_samples = 1000;
path_loss_direct_set = 0.1:0.1:2;
num_irs_elements_set = [100,120,140,160];
num_user = 7;
irs_Nh_set = [10,10,10,10];
Rician_factor = 10;
scale_factor = 120;
sigma2_dB = -10;
...
l=plot(eta_set(:,1),prob_set(1,:),'o-');hold on
l.MarkerFaceColor = l.Color;
l=plot(eta_set(:,2),prob_set(2,:),'d-');hold on
l.MarkerFaceColor = l.Color;
l=plot(eta_set(:,3),prob_set(3,:),'d-');hold on
l.MarkerFaceColor = l.Color;
l=plot(eta_set(:,4),prob_set(4,:),'d-');hold on
l.MarkerFaceColor = l.Color;
xlabel('$\eta$ ','Interpreter','latex')
ylabel('Empirical feasible probability','Interpreter','latex')
legend('$N=100$','$N=120$','$N=140$','$N=160$','Interpreter','latex','Location','southeast')
grid on
hold on
